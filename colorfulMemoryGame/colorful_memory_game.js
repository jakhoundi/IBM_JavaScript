...
/*
colors array: This array holds distinct color values in strings, 
representing the colors for the cards in the memory match game. 
...
selectedCards: This variable acts as a temporary storage for the 
currently selected cards during the game. When a player clicks on a card, 
it gets added to this array to enable match comparisons.
...
gameInterval: This variable manages the game timer. It's utilized to control the
 countdown mechanism for the game's duration. The interval continuously 
 decrements the 'timeLeft' variable, updating the displayed time and triggering 
 the game's end when the time expires.
*/
...
It typically represents a button element intended to start or restart the game
 when clicked. This variable allows the JavaScript code to access and manipulate
  this specific button element.

gameContainer: This variable is assigned the HTML element with the 
ID 'game-container'; it refers to a div or container element that dynamically 
generates cards for the memory match game. It allows JavaScript to manipulate 
or append child elements (cards) within this container.

scoreElement: This variable represents the HTML element with the ID 'score'. 
It is associated with a paragraph or span element displaying the player's 
score during the game. JavaScript can update the displayed score by 
manipulating this specific element's content.

timerElement: This variable refers to the HTML element identified by 
the ID 'timer'. It's presumably linked to a paragraph or span element 
that displays the time remaining for the player to complete the game. 
JavaScript can update this element to reflect the countdown and notify the 
player about the remaining time.
*/
...
/*
Create the generateCards() function responsible for dynamically creating 
the card elements within the game container based on the 'cards' 
array that holds color values for the cards. This function creates the 
card elements dynamically within the game-container div. Include given 
code in javaScript file after previous code.
